...
function isYearOnlyLabel(label) {
  ...
  // 4-digit year
  if (/^\d{4}$/.test(s)) return Number(s);
  // 2-digit year support --> buggy with 1990s. only works with 2000s
...
const UNIT_DEFS = {
  ...
  ft:  { dim: "height",  toBase: v => v,        fromBase: v => v },
...
function detectUnitsFromCsvMetadataLine(line) {
  ...
  if (t === "foot" || t === "feet" || t === "ft") return "ft";
...
function convertForDisplay(rawValue, { desiredUnits, sourceUnits, colName }) {
  ...
  if (!src) {
    if (want === "AF" || want === "TAF") {
      ...
      // try to infer from the column/header text too
      src = normalizeUnits(colName, null);
    }

    if (!src) {
      throw new Error(
        `Missing source units for desired "${want}". ` +
        `Add data-source-units / sourceUnits or ensure CSV contains a units metadata line.`
      );
    }
  }
...
//       throw new Error(
//         `Missing source units for desired "${want}". ` +
//         `Add data-source-units="IN" / "CFS" / "FT" or ensure CSV contains a units metadata line.`
//       );
//     }
//   }
...
function unitAxisLabel(units) {
  ...
  if (u === "ft") return "Height (ft)";
  if (u === "cfs") return "Streamflow (cfs)";
  return `Value (${u})`;
}
...
function buildStackForecastOverlays(baseSeries, splitIdx) {
  const overlays = [];

  const stackedAreas = baseSeries.filter(
    s => s.stack === "total" && s.areaStyle && s.name !== "__internal_stack_overlay__"
  );
  ...
  stackedAreas.forEach((areaSeries, index) => {
    ...
    const bridgeIdx = Math.max(0, splitIdx - 1);
...
// --------- Monthly Change Calculation Helper Function ----------
function computeMonthlyChange(values) {
  ...
  Promise.all(seriesConfig.map(s => fetchText(s.csv)))
    .then(csvTexts => {
      ...
      if (cfg.monthlyChange === true) {
        ...
        const yChanged = computeMonthlyChange(y);
      ...
      const makeSeries = (name, data, { dashed } = {}) => ({
        ...
        itemStyle: chartType === "bar"
          ? {
              ...
              // use makestripepattern if dashed/forecasted
              // color: dashed
              //         ? makeStripePattern(color || "#5B8FF9")
              //         : (color || "#5B8FF9"),

              borderColor: color || "#4b5563",
              borderWidth: dashed ? 2 : 1,
              borderType: dashed ? "dashed" : "solid",
        ...
        custom: { units: desiredUnits, yAxisIndex: Number(cfg.yAxisIndex) || 0 },
      });
      ...
      // Keep continuity for line/area series only; bars should not overlap at the boundary.
      if (chartType !== "bar" && splitIdx > 0) {
        futureConnected[splitIdx - 1] = y[splitIdx - 1];
      }
    ...
    if (stacked) {
      ...
      // const yAxisName = seriesUnits.length === 1 ? unitAxisLabel(seriesUnits[0]) : "Value";
      // console.log("Series units:", seriesUnits, "yAxisName:", yAxisName);
      const visibleSeries = series.filter(s => s.name !== "__internal_stack_overlay__");

      const primarySeries = visibleSeries.filter(s => (s.yAxisIndex || 0) === 0);
      const secondarySeries = visibleSeries.filter(s => (s.yAxisIndex || 0) === 1);

      const primaryUnits = Array.from(new Set(primarySeries.map(s => s._units).filter(Boolean)));
      const secondaryUnits = Array.from(new Set(secondarySeries.map(s => s._units).filter(Boolean)));

      const primaryAxisName = el.dataset.yAxisName || (primaryUnits.length === 1 ? unitAxisLabel(primaryUnits[0]) : "Value");
      const secondaryAxisName = el.dataset.yAxisNameSecondary || (secondaryUnits.length === 1 ? unitAxisLabel(secondaryUnits[0]) : "Value");
      ...
      chart.setOption({
        ...
        // graphic: hasForecastSplit ? [
        //   {
        //     type: "text",
        //     left: "center",
        //     bottom: 30, // sits below legend (legend bottom is 60)
        //     style: {
        //       text: "Solid lines represent observed values; Dashed lines represent forecasted values",
        //       fontSize: 12,
        //       fill: "#898e98c4", // muted gray to match your CSS var(--muted)
        //       fontWeight: 10
        //     }
        //   }
        // ]: [],
        ...
        tooltip: {
          ...
          formatter: (params) => {
            ...
            if (showConditionBands && month) {
              const conditionName = conditionLookup.get(month);
              if (conditionName) {
                rows.unshift(`
                  <div style="display:flex;align-items:center;gap:8px;">
                    <span style="width:10px;height:10px;border-radius:2px;background:${CONDITION_COLORS[conditionName]};display:inline-block;"></span>
                    <span style="font-weight:600;">${conditionName}</span>
                  </div>`);
              }
            }
...
function mountSingleValuesInCharts(root = document) {
  const chartBuckets = new Map();

  root.querySelectorAll('.single-value[data-in-chart]').forEach((el) => {
    const target = (el.dataset.inChart || '').trim();
    if (!target) return;

    const chartEl = root.querySelector(target);
    if (!chartEl || !chartEl.classList.contains('chart')) return;

    if (!chartBuckets.has(chartEl)) {
      chartBuckets.set(chartEl, []);
    }
    chartBuckets.get(chartEl).push(el);
  });

  chartBuckets.forEach((cards, chartEl) => {
    let inlineRow = chartEl.querySelector('.chart-inline-values-row');
    if (!inlineRow) {
      inlineRow = document.createElement('div');
      inlineRow.className = 'chart-inline-values-row';
      chartEl.appendChild(inlineRow);
    }

    cards.forEach((el) => {
      el.classList.add('single-value-in-chart');
      inlineRow.appendChild(el);
    });

    chartEl.classList.add('chart-has-inline-value');

    // ECharts needs an explicit resize after late DOM/layout changes.
    if (window.echarts && typeof window.echarts.getInstanceByDom === 'function') {
      const chartInstance = window.echarts.getInstanceByDom(chartEl);
      if (chartInstance) {
        requestAnimationFrame(() => chartInstance.resize());

        // Ensure one more resize after any CSS/layout settling.
        const resizeOnceAfterTransition = () => {
          chartInstance.resize();
          chartEl.removeEventListener('transitionend', resizeOnceAfterTransition);
        };

        chartEl.addEventListener('transitionend', resizeOnceAfterTransition, { once: true });
        setTimeout(() => chartInstance.resize(), 280);
      }
    }
  });
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // 3) optional: place selected single-value cards inside existing chart cards
  mountSingleValuesInCharts();
});
